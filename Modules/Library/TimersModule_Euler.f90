...
CONTAINS
  ...
  SUBROUTINE FinalizeTimers_Euler &
    ...
    CHARACTER(64) :: OverallTimeGPU = '(12x,A,ES13.6E3,A,F6.3,A,F7.3,A)'
    CHARACTER(64) :: TimeGPU        = '(14x,A,ES13.6E3,A,F6.3,A,F6.3,A)'
    ...
    IF( Verbose )THEN
      ...
      WRITE(*,*)
      WRITE(*,TRIM(Label_Level2)) 'GPU-specific'
      WRITE(*,TRIM(Label_Level2)) '------------'
      TotalTime = Timer_Euler_CopyIn &
                    + Timer_Euler_Permute &
      ...
      WRITE(*,'(52x,A)') '%DG'
      WRITE(*,TRIM(OverallTimeGPU)) &
        'Timers-GPU = ', TotalTime, ' s = ', &
        100.0_DP * TotalTime / Timer_Euler_Program, ' % === ', &
        100.0_DP * TotalTime / Timer_Euler_dgDiscretization, ' %'
      WRITE(*,*)
      ...
      WRITE(*,TRIM(TimeGPU)) &
        'Interpolate:      ', &
        Timer_Euler_Interpolate, ' s = ', &
        100.0_DP &
          * Timer_Euler_Interpolate / Timer_Euler_Program, ' %'
